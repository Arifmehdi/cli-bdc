...
// $autoWebMakeFilterMakeInput = $searchData['autoWebMakeFilterMakeInput'] ?? '';
$webExteriorColorFilter = isset($searchData['webExteriorColorFilter']) ? (array) $searchData['webExteriorColorFilter'] : [];
$webInteriorColorFilter = isset($searchData['webInteriorColorFilter']) ? (array) $searchData['webInteriorColorFilter'] : [];
...
// exterior color code Start here *****************************
// Fetch filters from cookies
$webExteriorColorFilter = $searchData['webExteriorColorFilter'] ?? [];
$webInteriorColorFilter = $searchData['webInteriorColorFilter'] ?? [];
...
if (!function_exists('getColorChecks')) {
    function getColorChecks(array $colors, array $selectedColors)
    {
        ...
        $allColorsChecked = empty($selectedColors) || !array_filter($colors, function ($color) use ($selectedColors) {
            return in_array($color, $selectedColors);
        });

        // Ensure 'all' key exists and is set to 'checked' if all colors are selected
...
// Get checked statuses for exterior and interior colors
$webExteriorColors = getColorChecks($exteriorColors, (array) $webExteriorColorFilter);
$webInteriorColors = getColorChecks($interiorColors, (array) $webInteriorColorFilter);
// Get checked statuses for exterior and interior colors
$mobileExteriorColors = getColorChecks($exteriorColors, (array) $webExteriorColorFilter);
$mobileInteriorColors = getColorChecks($interiorColors, (array) $webInteriorColorFilter);
// // interior color code End here ***********************8********
...
// $webFuelTypeFilter = isset($searchData['webFuelTypeFilter']) ? (array) $searchData['webFuelTypeFilter'] : [];
// // Alternatively, you can use the null coalescing operator like this:
$webFuelTypeFilter = $searchData['webFuelTypeFilter'] ?? [];

// Function to generate checked statuses
if (!function_exists('getFuelTypeChecks')) {
    function getFuelTypeChecks(array $fuelTypes, array $selectedFuelTypes)
    {
        // Check if 'all' fuel types are selected
        $allFuelTypesChecked = empty($selectedFuelTypes) || !array_filter($fuelTypes, function ($fuelType) use ($selectedFuelTypes) {
            return in_array($fuelType, $selectedFuelTypes);
        });

        // Ensure 'all' key exists and is set to 'checked' if all fuel types are selected
        $checkedStatuses = ['all' => $allFuelTypesChecked ? 'checked' : ''];

        // Check each fuel type individually
        foreach ($fuelTypes as $fuelType) {
            $checkedStatuses[$fuelType] = in_array($fuelType, $selectedFuelTypes) ? 'checked' : '';
        }
...
$webExteriorColors = getColorChecks($exteriorColors, (array) $webExteriorColorFilter);
$webInteriorColors = getColorChecks($interiorColors, (array) $webInteriorColorFilter);
// // fuel code End here ***********************8********

// vehicles_fuel code start here ****************************************
if (!function_exists('getFuelChecks')) {
    function getFuelChecks(array $fuelTypes, array $selectedFuels)
    {
        $allFuelsChecked =
            empty($selectedFuels) ||
            !array_filter($fuelTypes, function ($fuel) use ($selectedFuels) {
...
// $vehicles_fuel = ['Gas', 'Diesel', 'Electric', 'Hybrid', 'Flex Fuel', 'Gasoline','Biodiesel','Flex Fuel Vehicle','N/A','Fuel Cell','Compressed Natural Gas','Plug-In Hybrid','----','–','E85 Flex Fuel','Hybrid Fuel','Regular Unleaded','Flexible Fuel','Gaseous','Plug-in Gas/Electric Hybrid'];
$vehicles_fuel = $cachefuel;
$mobileFuelChecks = getFuelChecks($vehicles_fuel, (array) $autoMobileFuelCheckbox);
$webFuelChecks = getFuelChecks($fuelTypes, (array) $autoWebFuelCheckbox);
...
<div style="margin-top: 90px" class="mobile-auto-top">
    <div class="container">
        <div class="">
            ...
            <h1 style="font-size: 1.5em; font-weight: 600; margin: 0;">Cars For Sale</h1>
        </div>
    </div>
</div>
<!--/Breadcrumb-->
                                    ...
                                    <div class="p-3">
                                        <div class="mb-3 form-group">
                                            ...
                                            <select id="webMakeFilterMakeInput"
                                                ...
                                                <option value="{{ $vehiclMakeData }}"
                                                    {{-- ($make_data == $vehiclMakeData) ? 'selected' : (($webMakeFilterMakeInput == $vehiclMakeData) ? 'selected' : '') --}}
                                        ...
                                        <div class="custom-row">
                                            ...
                                            <div
                                                ...
                                                <a href="javascript:void(0)"
                                                    ...
                                                    <p style="text-align:center; margin-top:4px; margin-bottom:0px">
                                                        {{ $bodyType }}
                                                    </p>
                                                </a>
                                            </div>
                                    ...
                                    <div class="filter-product-checkboxs">
                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                            <input type="checkbox" class="custom-control-input me-2 common_selector"
                                                ...
                                                id="selectAllWebExteriorColor" {{ $webExteriorColors['all'] ?? '' }} selected>
                                        ...
                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                            <input type="checkbox" class="custom-control-input me-2 autoWebExteriorColorCheckbox common_selector"
                                                name="checkbox{{ $exteriorColor }}" value="{{ $exteriorColor }}" data-value="{{ $exteriorColor }}"
                                                {{ $webExteriorColors[$exteriorColor] ?? '' }}>
                                            <span class="circle-color me-2" style="background-color: {{ $exteriorColor }}; width: 20px; height: 20px; display: inline-block; border-radius: 50%; border: 1px solid #C0BDBD;"></span>
                                    ...
                                    <div class="filter-product-checkboxs">
                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                            <input type="checkbox" class="custom-control-input me-2 common_selector"
                                                ...
                                                id="selectAllWebInteriorColor" {{ $webInteriorColors['all'] ?? '' }} selected>
                                            <span class="custom-control-label ms-1">All</span>
                                        </label>
                                        ...
                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                            <input type="checkbox" class="custom-control-input me-2 autoWebInteriorColorCheckbox common_selector"
                                                name="checkbox{{ $interiorColor }}" value="{{ $interiorColor }}" data-value="{{ $interiorColor }}"
                                                {{ $webInteriorColors[$interiorColor] ?? '' }}>
                                            <span class="circle-color me-2" style="background-color: {{ $interiorColor }}; width: 20px; height: 20px; display: inline-block; border-radius: 50%; border: 1px solid #ccc;"></span>
                                ...
                                <div class="accordion-body">
                                    ...
                                    <label class="mb-2 custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input common_selector"
                                            ...
                                            @if(isset($webFuelTypes['all']) && $webFuelTypes['all'] == 'checked') checked @endif >
                                        <span class="custom-control-label">All</span>
                                    </label>
                                        @foreach ($fuelTypes as $fuelType)
                                            <label class="mb-2 custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input autoWebFuelCheckbox common_selector"
                                                    name="checkboxweb11" value="{{ $fuelType }}"
                                                    @if(isset($webFuelTypes[$fuelType]) && $webFuelTypes[$fuelType] == 'checked') checked @endif>
                    ...
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="col-xl-3 col-lg-3 col-md-12">
                                        ...
                                        <div class="accordion-item">
                                            ...
                                            <div style="border-left: 1px solid rgb(206, 205, 205); border-right: 1px solid rgb(206, 205, 205); border-bottom: 1px solid rgb(206, 205, 205)"
                                                ...
                                                <div class="accordion-body">
                                                    <div class="filter-product-checkboxs">
                                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                                            <input type="checkbox" class="custom-control-input me-2 mobile_common_selector"
                                                                name="allMobileExteriorColorName" value="All"
                                                                id="selectAllMobileExteriorColor" {{ $mobileExteriorColors['all'] ?? '' }} selected>
                                                            <span class="custom-control-label ms-1">All</span>
                                                        </label>

                                                        <input type="hidden" value="" id="mobileExteriorColor">

                                                        @foreach ($exteriorColors as $exteriorColor)
                                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                                            <input type="checkbox" class="custom-control-input me-2 filter-product-checkboxs autoMobileExteriorColorCheckbox common_selector"
                                                                name="checkbox{{ $exteriorColor }}" value="{{ $exteriorColor }}" data-value="{{ $exteriorColor }}"
                                                                {{ $mobileExteriorColors[$exteriorColor] ?? '' }}>
                                                            <span class="circle-color me-2" style="background-color: {{ $exteriorColor }}; width: 20px; height: 20px; display: inline-block; border-radius: 50%; border: 1px solid #ccc;"></span>
                                                            <span class="custom-control-label ms-1">{{ $exteriorColor }}</span>
                                                        </label>
                                                        @endforeach
                                                    </div>
                                        ...
                                        <div class="accordion-item">
                                            ...
                                            <div style="border-left: 1px solid rgb(206, 205, 205); border-right: 1px solid rgb(206, 205, 205); border-bottom: 1px solid rgb(206, 205, 205)"
                                                ...
                                                <div class="accordion-body">
                                                    <div class="filter-product-checkboxs">
                                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                                            <input type="checkbox" class="custom-control-input mobile_common_selector"
                                                                name="allMobileInteriorColorName" value="All"
                                                                id="selectAllMobileInteriorColor" {{ $mobileInteriorColors['all'] ?? '' }} selected>
                                                            <span class="custom-control-label ms-1">All</span>
                                                        </label>

                                                        <input type="hidden" value="" id="mobileInteriorColor">
                                                        
                                                        @foreach ($interiorColors as $interiorColor)
                                                        <label class="mb-2 custom-control custom-checkbox d-flex align-items-center">
                                                            <input type="checkbox" class="custom-control-input filter-product-checkboxs autoWebInteriorColorCheckbox common_selector"
                                                                name="checkbox{{ $interiorColor }}" value="{{ $interiorColor }}" data-value="{{ $interiorColor }}"
                                                                {{ $webInteriorColors[$interiorColor] }}>
                                                            <span class="circle-color me-2" style="background-color: {{ $interiorColor }}; width: 20px; height: 20px; display: inline-block; border-radius: 50%; border: 1px solid #ccc;"></span>
                                                            <span class="custom-control-label ms-1">{{ $interiorColor }}</span>
                                ...
                                <div style="padding: 0; height: auto; margin-bottom: 15px" class="card-body">
                                    <div class="accordion accordion-flush" id="accordionFlushExample-drivetrain-mobile">
                                        <div class="accordion-item">
                                            <h4 style="border: 0.02px solid rgb(206, 205, 205);"
                                                class="accordion-header" id="flush-headingOne">
                                                <button class="accordion-button collapsed" type="button"
                                            ...
                                            <div style="border-left: 1px solid rgb(206, 205, 205); border-right: 1px solid rgb(206, 205, 205); border-bottom: 1px solid rgb(206, 205, 205); "
                                                id="flush-collapseOne-drivetrain-mobile" class="accordion-collapse collapse"
                                                aria-labelledby="flush-headingOne"
                                                    ...
                                                    <div class="filter-product-checkboxs">
                                                        <label class="mb-2 custom-control custom-checkbox">
                                                            <input type="checkbox"
                                                                ...
                                                                @if(isset($mobileFuelChecks['all']) && $mobileFuelChecks['all'] == 'checked') checked @endif >
                                                            <span class="custom-control-label">
                                                                All
                                                            </span>
                                                        </label>
                                                        ...
                                                        <label class="mb-2 custom-control custom-checkbox">
                                                            <input type="checkbox"
                                                                ...
                                                                @if(isset($mobileFuelChecks[$fuelType]) && $mobileFuelChecks[$fuelType] == 'checked') @endif>
                                                            <span
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const zipCode = urlParams.get("zip"); // Get zipcode parameter
