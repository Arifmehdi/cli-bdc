...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        $('#web_radios').on('change', function() {
            ...
            var radiusParamData = $('#web_radios').val();
            // alert(radiusParamData);
            if(radiusParamData != 'Nationwide'){
                if (!selectZipCode) {
                    $('#web_radios').val('');
                    Swal.fire({
                        icon: 'warning',
    ...
    $(document).ready(function(){
        ...
        if (screenWidth < 1000) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    $(window).scrollTop(0);
                    let totalCount = res.total_count; // Assuming res.total_count contains the count
                    let formattedCount = totalCount.toLocaleString(); // Formats number with commas

                    // Check if totalCount is exactly 10000
                    $('#web_sticky_btn').text(formattedCount + (totalCount === 10000 ? '+' : ''));
