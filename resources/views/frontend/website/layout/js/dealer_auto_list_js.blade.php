...
<!-- active filter start here active filter start here  active filter start here  active filter start here active filter start here active filter start here -->
@include('frontend.website.layout.js.active_filter_js')
<!-- active filter end here  active filter end here  active filter end here  active filter end here  active filter end here  active filter end here -->

<script>
    $(document).ready(function() {
        // //move into below DomContentLoaded event listener
        ...
        // check for zip code start here check for mobile radius
        $('#mobile_location').on('change', function() {
            var mobile_radios = $('#mobile_radios').val();
            var mobileZipCode = $(this).val();
            updateUrlParameter('zip', mobileZipCode);

            if (!mobileZipCode && mobile_radios != 'Nationwide') {
        ...
        $('#web_radios').on('change', function() {
            ...
            if (!selectZipCode && newRadiusCode != 'Nationwide') {
                ...
                Swal.fire({
                    icon: 'warning',
                    title: 'Oops...',
                    text: 'Please enter your zip code first to view vehicle listings near you.',
                    confirmButtonText: 'OK',
                    confirmButtonColor: '#4caf50',
                    background: '#f4f6f7',
                    customClass: {
        ...
        var minPriceData =    {{ app('priceRange')['used']['minPrice'] }};
        var maxPriceData =    {{ app('priceRange')['used']['maxPrice'] }};


        var minPriceRate = minPriceData; //min price value
        var maxPriceRate = maxPriceData; //max price value
        var initialMinDisplayValue = minPriceRate;
        var initialMaxDisplayValue = maxPriceRate;
        ...
        var minValue = minPriceRate;
        var maxValue = maxPriceRate;

        var defaultMinValue = minPriceRate;
        var defaultMaxValue = maxPriceRate;
        ...
        var priceMaxValue = searchData && searchData.rangerMaxPriceSlider ? maxPriceRate : defaultMaxValue;
        // alert(priceMaxValue);
        ...
        function updatePriceRangeDisplay(minValue, maxValue) {
            ...
            priceRangeDisplay.textContent = (displayMinValue === 0 ? '0' : priceFormatNumber(displayMinValue)) + ' – ' + (displayMaxValue === initialMaxDisplayValue ? '300000+' : priceFormatNumber(displayMaxValue));
        }
        ...
        var initialMaxDisplayValue = 300000;
        ...
        var defaultMaxPrice = 300000; // Default maximum price
        ...
        var initialMaxMileageDisplayValue = 300000;
        ...
        var defaultMaxMiles = 300000; // Default maximum mileage
        ...
        var initialMinYearDisplayValue = 1900;
        ...
        var yearMinValue = searchData ? (searchData.rangerYearMinPriceSlider || 1900) : 1900;
        ...
        if (searchData) {
            yearNonLinearSlider.noUiSlider.set([
                searchData.rangerYearMinPriceSlider || 1900,
        ...
        var mobileYearMinValue = searchData ? (searchData.mobileYearRangerMinPriceSlider || 1900) : 1900;
        ...
        $(document).on('click', '.common_selector_btn', function() {
            pageLoad();
        });
        ...
        $('.mobile_common_selector').on('change', function() {
            pageLoad();
        });
        ...
        function pageLoad(page = 1) {
            ...
            $(document).ready(function() {
                ...
                let homeMaxPrice = getUrlParameter('maximum_price');
                ...
                if (screenWidth < 1000) {
                    ...
                    var autoMobileInteriorColorCheckbox = get_filter('autoMobileInteriorColorCheckbox');
                    ...
                    var vinData = @json($vinArray);


                    $.ajax({
                        url: "{{ route('dealer') }}?page=" + page,
                        type: "get",
                        data: {
                            ...
                            autoMobileInteriorColorCheckbox: autoMobileInteriorColorCheckbox,
                            mobileBody: mobileBody,
                            mobilelocation: mobilelocation,
                            mobileRadios: mobileRadios,
                            selected_sort_search: selected_sort_search,

                            // url parameter for mobile
                            homeMaxPrice: homeMaxPrice,
                            vinData: vinData,
                        },
                    ...
                    var hfuel = getUrlParameter('hfuel');
                    loader;
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const selectAllWebFuelCheckbox = document.getElementById('selectAllFuel');
...
<script>
    $(document).ready(function() {
        // Function to check default selection
        function checkDefault(selectAllId, checkboxesClass) {
            if ($(checkboxesClass + ':checked').length === 0) {
                $(selectAllId).prop('checked', true);
            }
        }

        // Generic function to manage checkbox behavior
        function manageCheckbox(selectAllId, checkboxesClass) {
            // Handle "All" checkbox
            $(selectAllId).change(function() {
                if ($(this).prop('checked')) {
                    $(checkboxesClass).prop('checked', false);
                }
            });
            ...
            $(checkboxesClass).change(function() {
                if ($(checkboxesClass + ':checked').length > 0) {
                    ...
                    checkDefault(selectAllId, checkboxesClass); // If none selected, select "All"
                }
            });

            // Initial check on page load
            checkDefault(selectAllId, checkboxesClass);
        }

        // Manage Fuel Checkboxes
        manageCheckbox('#selectAllFuel', '.autoWebFuelCheckbox');
        // Manage Drivetrain Checkboxes
        manageCheckbox('#selectAllWebDriveTrain', '.autoWebDriveTrainCheckbox');
        // Manage Exterior Color Checkboxes
        manageCheckbox('#selectAllWebExteriorColor', '.autoWebExteriorColorCheckbox');
        // Manage Interior Color Checkboxes
        manageCheckbox('#selectAllWebInteriorColor', '.autoWebInteriorColorCheckbox');
        // Manage Transmission Checkboxes
        manageCheckbox('#selectAllWebTransmissionCheckbox', '.autoWebTransmissionCheckbox');
        // Manage Condition Checkboxes
        manageCheckbox('#selectAllWebConditionCheckbox', '.autoWebConditionCheckbox');


        // Manage mobile device condition / type Checkboxes
        manageCheckbox('#selectAllCheckbox', '.autoMobileTypeCheckbox');
        // Manage mobile device tranmission Checkboxes
        manageCheckbox('#selectAllTranscissionCheckbox', '.autoMobileTransmissionCheckbox');
        // Manage mobile exterior color Checkboxes
        manageCheckbox('#selectAllMobileExteriorColor', '.autoMobileExteriorColorCheckbox');
        // Manage mobile interior color Checkboxes
        manageCheckbox('#selectAllMobileInteriorColor', '.autoWebInteriorColorCheckbox');
        // Manage mobile drivetrain Checkboxes
        manageCheckbox('#selectAllMobileDriveTrain', '.autoMobileDriveTrainCheckbox');
        // Manage mobile fuel Checkboxes
        manageCheckbox('#selectAllFuelCheckbox', '.autoMobileFuelCheckbox');
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const zipParamData = urlParamsData.get("zip"); // Get zip code from URL
        let radiusParamData = urlParamsData.get("radius"); // Get radius from URL

        var cookieZipcode = @json($cookie_zipcode);

        // Select the distance dropdown element
        const radiusDropdownData = document.getElementById("web_radios");
        const mobileRadiusDropdownData = document.getElementById("mobile_radios");

        // If zip exists but radius is not set, default to 75
        if (zipParamData && !radiusParamData) {
            radiusParamData = "75";
            updateUrlParameter('radius', radiusParamData);
        }

        if (zipParamData == null && cookieZipcode) {
            radiusParamData = "75";
            updateUrlParameter('radius', radiusParamData);
        }
        ...
        selectRadiusOption(radiusDropdownData, radiusParamData);
        selectRadiusOption(mobileRadiusDropdownData, radiusParamData);
        ...
        let observer = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    img.src = img.dataset.src; // Load the image when it enters the viewport
