...
@php
    // Extract all values first to avoid complex nested ternaries
    $dealer = is_array($inventory) ? ($inventory['dealer'] ?? null) : ($inventory->dealer ?? null);
    $additional_inventories = is_array($inventory) ? ($inventory['additional_inventory'] ?? null) : ($inventory->additionalInventory ?? null);

    $id = is_array($inventory) ? ($inventory['id'] ?? '') : ($inventory->id ?? '');
    $user_id = is_array($inventory) ? ($inventory['user_id'] ?? '') : ($inventory->user_id ?? '');
    $year = is_array($inventory) ? ($inventory['year'] ?? '') : ($inventory->year ?? '');
    $make = is_array($inventory) ? ($inventory['make'] ?? '') : ($inventory->make ?? '');
    $model = is_array($inventory) ? ($inventory['model'] ?? '') : ($inventory->model ?? '');
    $trim = is_array($inventory) ? ($inventory['trim'] ?? '') : ($inventory->trim ?? '');
    $vin = is_array($inventory) ? ($inventory['vin'] ?? '') : ($inventory->vin ?? '');
    $type = is_array($inventory) ? ($inventory['type'] ?? '') : ($inventory->type ?? '');
    $fuel = is_array($inventory) ? ($inventory['fuel'] ?? '') : ($inventory->fuel ?? '');
    $transmission = is_array($inventory) ? ($inventory['transmission'] ?? '') : ($inventory->transmission ?? '');
    $bodyFormated = is_array($inventory) ? ($inventory['body_formated'] ?? '') : ($inventory->body_formated ?? '');
    $exteriorColor = is_array($inventory) ? ($inventory['exterior_color'] ?? '') : ($inventory->exterior_color ?? '');
    $interior_color = is_array($inventory) ? ($inventory['interior_color'] ?? '') : ($inventory->interior_color ?? '');
    $engine_details = is_array($inventory) ? ($inventory['engine_details'] ?? '') : ($inventory->engine_details ?? '');
    $miles = is_array($inventory) ? ($inventory['miles'] ?? '') : ($inventory->miles ?? '');
    $price = is_array($inventory) ? ($inventory['price'] ?? 0) : ($inventory->price ?? 0);
    $price_rating = is_array($inventory) ? ($inventory['price_rating'] ?? 0) : ($inventory->price_rating ?? 0);
    $payment_price = is_array($inventory) ? ($inventory['payment_price'] ?? 0) : ($inventory->payment_price ?? 0);
    $mpg_city = is_array($inventory) ? ($inventory['mpg_city'] ?? null) : ($inventory->mpg_city ?? null);
    $mpg_highway = is_array($inventory) ? ($inventory['mpg_highway'] ?? null) : ($inventory->mpg_highway ?? null);
    $drive_info = is_array($inventory) ? ($inventory['drive_info'] ?? null) : ($inventory->drive_info ?? null);
    $created_date = is_array($inventory) ? ($inventory['created_date'] ?? null) : ($inventory->created_date ?? null);

    $title = is_array($inventory) ? ($inventory['title']  ?? '') : ($inventory->title ?? '');

    $hasMiles = !empty($miles) && is_numeric(str_replace(',', '', $miles));
    $hasDealer = $dealer !== null;
    $hasAdditionalInventory = $additional_inventories !== null;

    if ($hasAdditionalInventory) {
        $seller_note = is_array($dealer) ? ($dealer['additional_inventory']['seller_note'] ?? '') : ($inventory->additionalInventory->seller_note ?? '');
        $local_img_url = is_array($dealer) ? ($inventory['additional_inventory']['local_img_url'] ?? '') : ($inventory->additionalInventory->local_img_url ?? '');
    }

    if ($dealer) {
        $mainDealerName = is_array($dealer) ? ($dealer['name'] ?? '') : ($dealer->name ?? '');
        $mainDealerCity = is_array($dealer) ? ($dealer['city'] ?? '') : ($dealer->city ?? '');
        $mainDealerState = is_array($dealer) ? ($dealer['state'] ?? '') : ($dealer->state ?? '');
        $mainDealerZip = is_array($dealer) ? ($dealer['zip'] ?? '') : ($dealer->zip ?? '');
        $addressLine1 = is_array($dealer) ? ($dealer['address_line_1'] ?? '') : ($dealer->address_line_1 ?? '');
        $addressLine2 = is_array($dealer) ? ($dealer['address_line_2'] ?? '') : ($dealer->address_line_2 ?? '');
        $dealerFullAddress = is_array($dealer) ? ($dealer['dealer_full_address'] ?? '') : ($dealer->dealer_full_address ?? '');
        $streetAddress = !empty($dealerFullAddress) ? $dealerFullAddress : trim($addressLine1 . ' ' . $addressLine2);
        $hasLocation = !empty($dealerCity) && !empty($dealerState);
        $dealer_rating = is_array($inventory['dealer']) ? ($inventory['dealer']['rating'] ?? null) : ($inventory->dealer->rating ?? null);
        $dealer_review = is_array($inventory['dealer']) ? ($inventory['dealer']['review'] ?? null) : ($inventory->dealer->review ?? null);
    }
...
@endphp
{
    ...
    "@type": "Vehicle",
    "name": "{{ addslashes($year) }} {{ addslashes($make) }} {{ addslashes($model) }} {{ $trim != 'N/A' ? addslashes($trim) : '' }}",
    "description": "{{ addslashes($custom_description ?? (app('globalSeo')['description'] ?? '')) }}",
    "image": "{{ $image_src ?? '' }}",
    "vehicleIdentificationNumber": "{{ $vin }}",
    "make": {
        "@type": "Brand",
        "name": "{{ addslashes($make) }}"
    },
    "model": "{{ addslashes($model) }}",
    @if(!empty($year))
    "dateVehicleFirstRegistered": "{{ $year }}-01-01",
    @endif
    "fuelType": "{{ addslashes($fuel) }}",
    "vehicleTransmission": "{{ addslashes($transmission) }}",
    "bodyType": "{{ addslashes($bodyFormated) }}",
    @if(!empty($exteriorColor))
    "color": "{{ addslashes($exteriorColor) }}",
    @endif
    @if($hasMiles)
    "mileageFromOdometer": {
        "@type": "QuantitativeValue",
        "value": {{ (int)str_replace(',', '', $miles) }},
        "unitText": "miles"
    },
    @endif
    "offers": {
        ...
        "seller": {
            "@type": "Organization",
            "name": "{{ addslashes($mainDealerName) }}",
            @if($hasLocation)
            "address": {
                "@type": "PostalAddress",
                "streetAddress": "{{ addslashes($streetAddress) }}",
                "addressLocality": "{{ addslashes($dealerCity) }}",
                "addressRegion": "{{ addslashes($dealerState) }}",
                "postalCode": "{{ addslashes($dealerZip) }}"
            }
            @endif
        }
        @endif
...
@php
    $custom_title = $title  . ' For Sale $' . number_format(floatval($price));
    ...
    if (is_array($inventory)) {
        $image_obj = $inventory['additional_inventory']['local_img_url'] ?? '';
    } else {
...
@section('title', ($year ?? '') . ' ' . ($make ?? '') . ' ' . ($model ?? '') . ' ' .
    ($trim != 'N/A' ? $trim : '') . ' for Sale in ' . ($idealerCity ?? '') . ', ' .
    strtoupper($dealerState ?? '') . ' - VIN ' . ($vin ?? '') . ' | BestDreamCar.com')
@section('content')
                        ...
                        <div style="background: rgb(255, 255, 255);" class="card-header w-100">
                            <div class="row  w-100 h-50 p-0 m-0 justify-content-between">
                                ...
                                <div class="col-xl-8 col-lg-7 col-md-8 col-sm-0 photo-collection">
                                    @if ($source)<span>{{ $source }}</span>@endif <span class="photos mt-3"><i class="fa fa-image  icon-icon-photos me-2"></i>Photos({{ $count }})</span>
                                </div>
                                <div style="text-align:center;" class="col-xl-4 col-lg-5 col-md-4 col-sm-12">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}&quote={{ $title }}"
                                            target="_blank">
                                            <img src="{{ asset('frontend/assets/social/fb.png') }}" alt="Facebook"
                                                class="social-icon mx-2" style="width: 24px; height: 24px; margin: 0 8px;">
                                        </a>

                                        <a href="https://x.com/intent/tweet?url={{ url()->current() }}&text={{ $title }}&via=bestdreamcar"
                                            target="_blank">
                                    ...
                                    @php
                                        ...
                                        foreach ($favourites as $favorite) {
                                            if (isset($favorite['id']) && $favorite['id'] == $id) {
                                                $countWishList = 1;
                                                break;
                                            }
                                        }
                                ...
                                <div style="width:97%; margin:0 auto" class="row mt-2 mb-3">
                                    <div thumbsSlider="" class="swiper mySwiper">
                                        ...
                                        <section>
                                            ...
                                            <h1 class="inventory-title">{{ $make_title_with_year }}</h1>
                                            <p style="line-height:1; font-size:14px">
                                                {{ number_format($miles) . ' miles' }}</p>
                                            <h4 style="line-height:1.1; font-size:20px ;font-weight:700">
                                                {{-- {{ $inventory->PriceFormate }} --}} ${{ number_format($price) }}

                                            @php
                                                // Initialize variables
                                                $totalPriceChange = 0;

                                                // Handle both array (cached) and object (database) formats
                                                $priceHistory = is_array($inventory)
                                                    ? ($inventory['main_price_history'] ?? [])
                                                    : ($inventory->mainPriceHistory ?? collect());

                                                // Convert to array if it's a collection
                                                $priceHistoryArray = is_object($priceHistory) ? $priceHistory->all() : $priceHistory;

                                                // Process price history
                                                foreach ($priceHistoryArray as $history) {
                                                    $changeAmount = is_array($history)
                                                        ? ($history['change_amount'] ?? '')
                                                        : ($history->change_amount ?? '');

                                                    if (strpos($changeAmount, '+') !== false || strpos($changeAmount, '-') !== false) {
                                                        $numericAmount = (float) str_replace(['$', ',', '+'], '', $changeAmount);
                                                        $totalPriceChange += $numericAmount;
                                                    }
                                                }
                                            ...
                                            @if($totalPriceChange != 0)
                                                <small style="font-size:12px">
                                                    <strong>
                                                        @if ($totalPriceChange < 0)
                                                            ${{ $displayAmount }} price drop
                                                        @elseif($totalPriceChange > 0)
                                                            ${{ $displayAmount }} price rise
                                                        @endif
                                                    </strong>
                                                </small>
                                            @endif
                                            ...
                                            <p
                                                ...
                                                EST .
                                                ${{ number_format($payment_price) ?? floor(number_format($global_cus_monthly_payment)) }}/mo*
                                            </p>
                                            <br>

                                            @if ($price_rating != null)
                                                @if ($price_rating == 'great-deal')
                                    ...
                                    <div class="tab-pane active" id="tab-1">
                                                    ...
                                                    <div style="margin-left:-12px !important"
                                                        ...
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            ...
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                ...
                                                                {{ str_replace('----', ' ', $mpg_city) }} City/
                                                                {{ str_replace('----', ' ', $mpg_highway) }}
                                                                Highway
                                                            </p>
                                                        </div>
                                                        ...
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            ...
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                class="auto-icon-para">VIN : {{ $vin }}</p>
                                                        </div>
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            title="Vehicle Type">
                                                            <img style="width:40px; height:40px; border:0.5px solid gray; border-radius:50%; padding:6px"
                                                            ...
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                class="auto-icon-para">Condition : {{ ucfirst($type) }}
                                                            </p>
                                                        </div>
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            title="Year">
                                                            ...
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                class="auto-icon-para">Year : {{ $year }}</p>
                                                        </div>
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            title="Location">
                                                            <img style="width:40px; height:40px; border:0.5px solid gray; border-radius:50%; padding:6px"
                                                    ...
                                                    <div style="margin-left:-12px" class="col-xs-12 col-md-6  col-sm-6 ">
                                                        ...
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            ...
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                ...
                                                                @if ($created_date)
                                                                    {{ (int) \Carbon\Carbon::parse($created_date)->diffInDays() }}
                                                                    Days ago
                                                                @endif
                                                            </p>
                                                        ...
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            ...
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                class="auto-icon-para">Model : {{ $model }}</p>
                                                        </div>
                                        ...
                                        @php
                                            // Sample data from the database
                                                if (is_array($inventory)) {
                                                    $description = $inventory['additional_inventory']['vehicle_additional_description'] ?? '';
                                                } else {
                                                    $description =  $inventory->additionalInventory->vehicle_additional_description;
                                                }
                                        ...
                                        @php
                                            if (is_array($inventory)) {
                                                $vehicle_feature_description = $inventory['additional_inventory']['vehicle_feature_description'] ?? '';
                                            } else {
                                                $vehicle_feature_description = $inventory->additionalInventory->vehicle_feature_description ?? '';
                                            }
                                        @endphp
                                                ...
                                                <div class="row p-2">
                                                    <div style="margin-left:-12px !important"
                                                        ...
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons "
                                                            ...
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                class="auto-icon-para">Trim : {{ $trim }}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div style="margin-left:-12px" class="col-xs-12 col-md-6  col-sm-6 ">
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            ...
                                                            <img style="width:40px; height:40px; border:0.5px solid gray; border-radius:50%; padding:6px"
                                                                src="{{ asset('/frontend/assets/images/model.png') }}" />
                                                            <p style="color:black; margin-left:10px; font-weight:600; margin-top:11px"
                                                                class="auto-icon-para">Make : {{ $make }}</p>
                                                        </div>
                                                        <div style="display: flex; margin-bottom:6px" class="boxicons"
                                                            title="Exterior Color ">
                                                            <img style="width:40px; height:40px; border:0.5px solid gray; border-radius:50%; padding:6px"
                                        ...
                                        <div class="mb-3">
                                            <div class="mb-4">
                                                <a href="#" class="text-dark">
                                                    <h4>
                                                        <strong>{{ $mainDealerName ?? explode(' in ', $dealerName)[0] }}</strong>
                                                    </h4>
                                                </a>
                                            </div>
                                            ...
                                            @php
                                                ...
                                                $averageRating = $dealer_rating ?? 4.7; // Replace with the actual rating (number or string)
                                                $reviews = $dealer_review ?? 43; // Replace with the actual review count
                                            ...
                                            <div class="mb-5">
                                                <span>{{ $formattedAddress ?? $dealerCity . ' ' . strtoupper($dealerState . ' ' . $dealerZip) }}</span>
                                            </div>
                                        </div>
                                        <div class="mt-5">
...
@php
    // Convert $relateds to array if it's a collection
    $relatedsArray = is_array($relateds) ? $relateds : $relateds->all();
    $relatedsCount = count($relatedsArray);
@endphp
...
@if ($relatedsCount > 1)
    <div class="hide-on-phone hide-on-tablet">
        ...
        <div id="myCarousel5" class="owl-carousel owl-carousel-icons3">
            @foreach ($relatedsArray as $item)

                @php
                    // Handle both array and object formats for $item
                    ...
                    $itemPaymentPrice = is_array($item) ? $item['payment_price'] : $item->payment_price;
                    $itemMiles = is_array($item) ? $item['miles'] : $item->miles;
                    $itemTransmission = is_array($item) ? $item['transmission'] : $item->transmission;
                    $itemVin = is_array($item) ? $item['vin'] : $item->vin;
                    $itemYear = is_array($item) ? $item['year'] : $item->year;
                    $itemMake = is_array($item) ? $item['make'] : $item->make;
                    $itemModel = is_array($item) ? $item['model'] : $item->model;
                    // $itemZip = is_array($item['dealer']) ? $item['dealer']['zip'] : $item->zip;
                    $itemZip = '';

                    if (is_array($item)) {
                        $itemZip = $item['dealer']['zip'] ?? $item['zip'] ?? '';
                    } elseif (is_object($item)) {
                        $itemZip = $item->dealer->zip ?? $item->zip ?? '';
                    }

                    // $itemDistance = is_array($item) ? ($item['distance'] ?? null) : ($item->distance ?? null);

                    // Handle dealer data
                    $dealer = is_array($item) ? ($item['dealer'] ?? []) : ($item->dealer ?? null);
                    $dealerName = is_array($dealer) ? ($dealer['name'] ?? '') : ($dealer->name ?? '');
                    $dealerCity = is_array($item) ? ($item['dealer']['city'] ?? '') : ($item->dealer->city ?? '');
                    $dealerState = is_array($item) ? ($item['dealer']['state'] ?? '') : ($item->dealer->state ?? '');

                    // Handle images
                    $additionalInventory = is_array($item) ? ($item['additional_inventory'] ?? []) : ($item->additionalInventory ?? null);
                    $image_obj = is_array($additionalInventory) ? ($additionalInventory['local_img_url'] ?? '') : ($additionalInventory->local_img_url ?? '');

                    $image_str = str_replace(['[', ' ', "'", ']'], '', $image_obj);
                    $images = array_filter(explode(',', $image_str));
                    $images_count = count($images);
                    $not_found_image = 'frontend/NotFound.png';

                    $vin_string_replace = str_replace(' ', '', $itemVin);
                    $route_string = str_replace(' ', '', $itemYear.'-'.$itemMake.'-'.$itemModel.'-in-'.$dealerCity.'-'.strtoupper($dealerState));

                    // Wishlist check
                    $countWishList = 0;
                    $favourites = session()->get('favourite', []);
                    foreach ($favourites as $favorite) {
                        if (isset($favorite['id']) && $favorite['id'] == $itemId) {
                            $countWishList = 1;
                            break;
                        }
                    }
                ...
                <div class="item">
                    <div style="background: rgb(255, 255, 255); background: linear-gradient(0deg, rgb(232, 245, 243) 0%, rgb(255, 255, 255) 100%);" class="card mb-0">
                        ...
                        <div class="item-card2-img">
                            @if ($images_count > 1)
                                ...
                                <img src="{{ asset($images[0]) }}" alt="img" class="auto-ajax-photo-details" loading="lazy" onerror="this.onerror=null; this.src='{{ asset($not_found_image) }}';">
                            @else
                                <a class="link" href="{{ route('auto.details', ['vin' => $vin_string_replace, 'param' => $route_string]) }}"></a>
                                <img src="{{ asset($not_found_image) }}" width="100%" alt="img" class="auto-ajax-photo-details-not-found">
                            @endif
                        </div>

                        <div class="item-card2-icons">
                            <div style="margin-top:-20px; margin-left:28px" class="item-card9-icons">
                                <a href="javascript:void(0);" class="item-card9-icons1" data-productid="{{ $itemId }}">
                                    @if ($countWishList > 0)
                                        <i class="fa fa-heart" style="color: red"></i>
                                    @else
                            ...
                            <div class="item-card2">
                                <div class="item-card2-desc">
                                    <div class="item-card2-text">
                                        @php $title = Str::limit($itemTitle, 25, '...'); @endphp
                                        <a href="{{ route('auto.details', ['vin' => $vin_string_replace, 'param' => $route_string]) }}" class="text-dark">
                                            <h4 style="font-weight:600" class="mb-0" title="{{ $itemTitle }}">{{ $title }}</h4>
                                        </a>
                                    </div>
                                    <div class="item-card9-desc mb-0 mt-1">
                                        @php $transmission = substr($itemTransmission, 0, 25); @endphp
                                        <p href="javascript:void(0);" class="me-4 d-inline-block mb-0"><span class="">{{ $transmission }}</span></p>
                                        <p class="mb-1" style="margin:0">
                                            @if (in_array($itemType, ['Preowned', 'Certified Preowned']))
                                                Used
                                            @else
                                                {{ $itemType }}
                                            @endif
                                    ...
                                    <div style="height: 25px" class="d-flex mb-1">
                                        <h4 class="me-3" style="font-weight:600">{{ $itemPrice }}</h4>
                                        <p style="color:black; font-weight:600; font-size:12px; margin-top:2px">${{ number_format($itemPaymentPrice) }}/mo*</p>
                                    </div>
                                    <div>
                                        <p class="d-flex mb-1">
                                            <img class="me-1" style="width:21px; height:21px; margin-top:0px" src="{{ asset('/frontend/assets/images/miles.png') }}" />
                                            {{ number_format($itemMiles) != 0 ? number_format($itemMiles).' miles' : 'TBD' }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-footer pe-4 ps-4 pt-4 pb-4">
                            <div class="item-card9-footer d-flex">
                                ...
                                @php
                                    $cus_dealer = explode(' in ', $dealerName)[0];
                                    $nameLength = strlen($cus_dealer);
                                    $name = Str::substr($cus_dealer, 0, 25);
                                @endphp
                                @if ($nameLength <= '25')
                                    <h5 class="dealer-add" style="color:black">{{ $cus_dealer }}<br>
                                        <span style="font-size:14px">{{ $dealerCity }}, {{ strtoupper($dealerState) }} {{ $itemZip  ?? ''}}
                                            @if (isset($itemDistance) && $itemDistance > 0.9)
                                                <span>({{ round($itemDistance, 0) }} mi. away)</span>
                                            @endif
                                        </span>
                                    </h5>
                                @else
                                    <h5 title="{{ $cus_dealer }}" class="dealer-add" style="color:black">{{ $name }}...<br>
                                        <span style="font-size:14px">{{ $dealerCity }}, {{ strtoupper($dealerState) }} {{ $itemZip  ?? ''}}
                                            @if (isset($itemDistance) && $itemDistance > 0.9)
                                                <span>({{ round($itemDistance, 0) }} mi. away)</span>
                                            @endif
                ...
                <div class="col-xl-5 col-lg-5 col-md-12">
                    ...
                    <i class="fa fa-copy"></i>
                        ...
                        <div class="card-body">
                            ...
                            <h5 class="card-title mt-2">
                                <span class="card-focus-header-year fw-bold">{{ $year }}</span>
                            ...
                            <!-- Price -->
                            <h3 class="fw-bold text-dark" style="font-size:2.5rem">${{ number_format($price) }}
                                <strong
                                    style="font-weight:300px; font-size:12px">${{ number_format($payment_price) ?? floor(number_format($global_cus_monthly_payment)) }}
                                    /mo* </strong>
                            </h3>
                    ...
                    alt="user"> --}}
                                <div>
                                    ...
                                    <span style="color:#e9e6e6" class=" ">{{ $mainDealerCity }},
                                        {{ strtoupper($mainDealerState) }}
                                        {{ $mainDealerZip }}</span>
                                                ...
                                                <div class="col-md-12 col-sm-12 col-xs-12 ">
                                                    <div class="form-group ">
                                                        <textarea style="border-radius:5px; border:1px solid rgb(189, 188, 188)" id="w3review"
                                                            class="form-control description" name="description" rows="6" cols="55">I am interested and want to know more about the {{ $title }}, you have listed for {{ $price }}  on Best Dream car.
                                                                                </textarea>
                                ...
                                <div style="border-radius:6px; box-shadow: rgba(100, 100, 111, 0.2) 0px 4px 7px 0px;"
                                    ...
                                    <div style="background: white; height:auto;" class="widget">
                                        <div class="financial-detail-card">
                                            ...
                                            <h2 style="display:block; text-align:center; color:white; font-weight:500">
                                                $<span class="ms-1"
                                                    id="calculator_monthly_pay">{{ number_format($payment_price) ?? floor(number_format($global_cus_monthly_payment)) }}</span>
                                            </h2>
                                            <p class="p-0 m-0"
                                                style="font-size:16px; display:block; text-align:center; color:rgb(235, 231, 231)"
...
@php
    ...
    $hasPriceHistory = count($priceHistoryArray) > 0;
@endphp
...
@if ($hasPriceHistory)
    <div class="card">
        <div class="card-body item-user">
            ...
            @php
                // Get created date
                $createdDate = is_array($inventory)
                    ? ($inventory['created_date'] ?? null)
                    : ($inventory->created_date ?? null);
            @endphp
            @if ($createdDate)
                <p class="mt-3 mb-0"> Listed
                    <strong>{{ (int) \Carbon\Carbon::parse($createdDate)->diffInDays() }} Days ago</strong>
                </p>
            @endif
            ...
            <div class="row p-0">
                <table class="table">
                    @foreach ($priceHistoryArray as $index => $history)
                        @php
                            $isLast = $index === count($priceHistoryArray) - 1;
                            $changeDate = is_array($history)
                                ? ($history['change_date'] ?? null)
                                : ($history->change_date ?? null);
                            $changeAmount = is_array($history)
                                ? ($history['change_amount'] ?? '')
                                : ($history->change_amount ?? '');
                            $amount = is_array($history)
                                ? ($history['amount'] ?? 0)
                                : ($history->amount ?? 0);
                        @endphp
                        ...
                        @if ($isLast)
                            <tr>
                                <th>{{ $changeDate ? \Carbon\Carbon::parse($changeDate)->format('m/d/Y') : '' }}</th>
                                <th>{{ $changeAmount }}</th>
                                <th>${{ number_format($amount, 0) }}</th>
                            </tr>
                        @else
                            <tr>
                                <td>{{ $changeDate ? \Carbon\Carbon::parse($changeDate)->format('m/d/Y') : '' }}</td>
                                <td>{{ $changeAmount }}</td>
                ...
                <p class="text-muted" style="font-size:12px">
                    <small>Total price amount is based on price change information provided by the seller</small>
                </p>
            </div>
        </div>
    </div>
@endif
                    ...
                    <style>
                     ...
                     {{-- jkdhgjksd gjkhsd sdfgj ksdj gsdlkjglksdjsg sdlkffgjlkdjglkd gkjsdlkgjlkdgjlkd sdjglkdjgkj dlkgjdlkfg lksdjgklsdjgk sdjgkjsd jsdkjglkdjglksdfjg sdjglksdjg lksdjglksdjgklsdj  --}}
                    {{-- @if ($relateds->count() > 1)
                    ...
                    @endif --}}
                    {{-- dokjglkdfjglkdjglkjdlkgjsdlkgjdlkgj lksdjglksdj glkdj glkjdfkgj okgjsdlkgj jlksdfgjlk sdgj gsdkjglk sdkgjdgjlksdjgk ksdjgksdjglksd lkgjdlkg sdkgjlksdjglksdjglksdjglksdjgk --}}
                                        ...
                                        <div class="col-md-12 col-sm-12 col-xs-12 ">
                                            <div class="form-group ">
                                                <textarea style="border-radius:5px; color:black" id="w3review" class="form-control description"
                                                    name="description" rows="6" cols="55">I am interested and want to know more about the {{ $title }}, you have listed for {{ $price }}  on Best Dream car.
                                                            </textarea>
                                        ...
                                        <div class="category-grid-box mt-3">
                                            ...
                                            <div class="ad-archive-img">
                                                ...
                                                @foreach ($images as $image)
                                                    <div class="swiper-slide">
                                                        <img src="{{ asset($image) }}"
                                                            alt="Used cars for sale {{ $title ?? '' }}, price is {{ $price ?? '' }}, vin {{ $vin ?? '' }} in {{ $inventory->dealer->city ?? '' }}, {{ strtoupper($inventory->dealer->state ?? '') }}, dealer name is {{ $inventory->dealer->name ?? '' }} Best Dream car image"
...
@push('js')
    ...
    <script>
        ...
        const priceHistory = @json($inventory->mainPriceHistory ?? $inventory['main_price_history']);
