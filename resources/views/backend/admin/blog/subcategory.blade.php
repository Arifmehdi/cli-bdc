            ...
            <div class="col-md-5">
                <div class="card">
                    <div class="card-header" id="creatModal">
                        <h3 class="card-title" id="header-title">Add Blog Sub Category</h3><br>
                        <hr>
                        <form action="#" method="post" id="userForm">
                            @csrf

                            <div class="form-group">
                                <label for="state">Blog Category Name</label>
                            ...
                            <div class="form-group">
                                <label for="state">Blog Sub-Category Name</label>
                                <input type="text" class="form-control" name="subcategory_name" id="subcategory_name"
                                    placeholder="Enter sub-category Name">
                                <span class="text-danger error-message" id="subcategory_name-error"></span>
                            </div>
            ...
            <div class="col-md-7">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Blog Sub Category List</h3>
...
$(document).ready(function() {
    $(function() {
        ...
        var table = $('.state-table').DataTable({
            ...
            searchable: true,
            "ajax": {
                "url": "{{ route('admin.blog.subcategory.index') }}",
                "datatype": "json",
                "dataSrc": "data",
                "data": function(data) {
                    //filter options
...
function dataStore() {
    ...
    $.ajax({
        url: "{{ route('admin.blog.subcategory.store') }}",
        type: 'post',
        data: {
...
function addform()
{
    var html = '<h3 class="card-title" id="header-title">Add Blog Category</h3><br><hr>' +
            '<form  method="post" id="userForm">@csrf' +
            '<div class="form-group">' +
            '<label for="make">Blog Category Name</label>' +
            '<input type="text" class="form-control" name="subcategory_name" id="subcategory_name" placeholder="Enter Category Name">' +
        ...
        $('#header-title').text('Add Blog Category')
}
...
$(document).ready(function() {
    ...
    $(document).on('click', '.delete', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('admin.blog.subcategory.destroy', '') }}" + '/' + id,
                    type: 'DELETE',
                    data: {
                        id: id
                    },
    ...
    $(document).on('click', '.edit', function(e) {
        ...
        $('#userForm').hide();
        $.ajax({
                url: "{{ url('admin/blog/subcategory') }}/" + id + "/edit",
                type: 'get',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(res) {
                    ...
                    // toastr.success(res.success);
                    $('.state-table').DataTable().draw(false);
                    $('#header-title').text('Edit Blog Category');

                    // Generate the HTML
                    var html = '<form method="post" id="editForm">@csrf' +
                            '<div class="form-group">' +
                    ...
                    categories.forEach(function(item) {
                        console.log('Category:', item.id, item.name); // Debug categories
                        html += '<option value="'+item.id+'" '+(item.id == res.category_id ? 'selected' : '')+'>'+item.name+'</option>';
                    });

                    html += '</select></div>' +
                            '<div class="form-group">' +
                            '<label>Sub Category</label>' +
                            '<input type="text" name="edit_subcategory_name" class="form-control" value="'+res.subCategoryName+'">' +
                            '<input type="hidden" name="idData" value="'+res.idData+'">' +
                            '</div>' +
                            '<div class="form-group">' +
    ...
    $.ajax({
        url: '{{ route("admin.blog.subcategory.update.v2")}}',
        type: 'post',
        data: formData,
        contentType: false,
        processData: false,
        success: function(res) {
            ...
            toastr.success('Blog Sub Category updated successfully');
            addform();
            $('.state-table').DataTable().draw(false);
        },
        error: function(error) {
