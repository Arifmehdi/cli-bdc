            ...
            <div class="col-md-5">
                <div class="card">
                    <div class="card-header" id="creatModal">
                        <h3 class="card-title" id="header-title">Add Blog Category</h3><br>
                        <hr>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.edit', function(e) {
        ...
        $('#userForm').hide();
        $.ajax({
                url: "{{ url('admin/blog/category') }}/" + id + "/edit",
                type: 'get',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(res) {
                    ...
                    var html = '<h3 class="card-title" id="header-title">Edit Blog Category</h3><br><hr>' +
                        '<form  method="post" id="editForm">@csrf' +
                        '<div class="form-group">' +
                        '<label for="state_name">Blog Category Name</label>' +
                        '<input type="text" class="form-control" name="edit_category_name" id="edit_category_name" placeholder="Edit Blog Category Name" value="'+res.categoryName+'">' +
    ...
    $.ajax({
        url: '{{ route("admin.blog.category.update.v2")}}',
        type: 'post',
        data: formData,
        contentType: false,
        processData: false,
