            ...
            <div class="col-md-5">
                <div class="card">
                    <div class="card-header" id="creatModal">
                        <h3 class="card-title" id="header-title">Add Vehicle Trim</h3><br>
                        <hr>
                        <form action="{{route('admin.trims.store')}}" method="post" id="userForm">
                            @csrf

                            <div class="form-group">
                                <label for="make">Trim Name</label>
                                <input type="text" class="form-control" name="trim_name" id="createTrim"
                                    placeholder="Enter Trim">
                                <span class="text-danger error-message" id="trim-error"></span>
                            </div>
                            <div class="form-group">
            ...
            <div class="col-md-7">
                ...
                <div class="card">
                    ...
                    <div class="card">
                        ...
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table  class="table table-bordered table-striped trim-table table-responsive">
                                <thead>
                                    <tr>
                                        <th>
                                            <div>
...
$(document).ready(function() {
    $(function() {
        ...
        var table = $('.trim-table').DataTable({
...
function dataStore() {
    ...
    var trim = $('#createTrim').val();
    var statusValue = $('input[name="status"]:checked').val();
    $.ajax({
        url: "{{ route('admin.trims.store') }}",
        type: 'post',
        data: {
            trim_name: trim,
            status: statusValue,
        ...
        success: function(response) {
            $('#userForm')[0].reset();
            $('.trim-table').DataTable().draw(false);
        ...
        error: function(error) {
            ...
            toastr.error(errors.make_name[0]);
            if (errors && errors.make) {
                $('#trim-error').text(errors.trim_name[0]);
...
function addform()
{
    var html = '<h3 class="card-title" id="header-title">Add Vehicle Make</h3><br><hr>' +
            ...
            '<label for="make">Trim Name</label>' +
            '<input type="text" class="form-control" name="trim_name" id="createTrim" placeholder="Enter Trim">' +
            '<span class="text-danger error-message" id="trim-error"></span>' +
            '</div>' +
            '<div class="form-group">' +
            '<label for="status">Status : </label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' +
        ...
        $('#header-title').text('Add Vehicle Trim')
}
...
$(document).ready(function() {
    ...
    $(document).on('click', '.delete', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('admin.trims.destroy', '') }}" + '/' + id,
                    type: 'DELETE',
                    data: {
                        id: id
                    },
    ...
    $(document).on('click', '.edit', function(e) {
        ...
        $.ajax({
                ...
                success: function(res) {
                    ...
                    // toastr.success(res.success);
                    $('.trim-table').DataTable().draw(false);
                    $('#header-title').text('Edit Vehicle Trim');

                    var html = '<h3 class="card-title" id="header-title">Edit Vehicle Make</h3><br><hr>' +
                        '<form  method="post" id="editForm">@csrf' +
                        '<div class="form-group">' +
                        '<label for="make">Trim Name</label>' +
                        '<input type="text" class="form-control" name="trim_name" id="editTrim" placeholder="Edit Trim" value="'+res.trimData+'">' +
    ...
    $(document).on('click','#editStoreBtn', function(e){
        ...
        var trim = $('#editTrim').val();
        var statusValue = $('input[name="status"]:checked').val();
        var updateRoute = '{{ route("admin.trims.update", ["trim" => ":id"]) }}';
        var url = updateRoute.replace(':id', id);
        $.ajax({
            url: url,
           type: 'PUT',
           data: {id:id, trim_name:trim, status:statusValue},
           success:function(res){
                console.log('res');
                toastr.success(res.success);
                addform();
