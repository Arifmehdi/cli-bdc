    ...
    public function auto(Request $request, $param = null)
{
    ...
    if ($clear == 'flush' || $clear == 'newCar') {
        ...
        return response()->json(['success' => $clear == 'flush' ? 'clear' : 'newcar']);
    }

    // Cache the MainInventory queries
    $cacheKey = 'main_inventory_data';
    $cacheDuration = now()->addMinutes(30); // Cache for 30 minutes

    $mainInventoryData = Cache::remember($cacheKey, $cacheDuration, function () {
    ...
    // Extract cached data
    $vehicles_body = $mainInventoryData['vehicles_body'];
    $vehicles_fuel_other = $mainInventoryData['vehicles_fuel_other'];
    $price_max = $mainInventoryData['price_max'];
    $price_min = $mainInventoryData['price_min'];
    $miles_max = $mainInventoryData['miles_max'];
    $miles_min = $mainInventoryData['miles_min'];

    // Get Vehicle Makes
    $vehicles = Cache::remember('vehicle_makes', $cacheDuration, function () {
    ...
    if ($request->ajax()) {
        ...
        // Cache the MainInventory query with dynamic filters
        $page = $request->input('page', 1); // Get the current page number
        $cacheKey = 'main_inventory_query_page_' . $page . '_' . md5(serialize($request->all()));
        $cacheDuration = now()->addMinutes(10); // Cache for 10 minutes

        
        $mainInventory = Cache::remember($cacheKey, $cacheDuration, function () use ($request, $dealer_id) {
            ...
            // Add distance calculation using selectRaw
            ...
            return $query->paginate(20); // Execute the query and cache the results
        });

        // Parse URL for additional filters
        ...
        if (isset($urlData['query'])) {
            ...
            if (isset($queryParams['homeDealerCitySearch']) && isset($queryParams['homeDealerStateSearch'])) {
                ...
                if (!empty($city_data)) {
                    $mainInventory->whereHas('dealer', function ($q) use ($city_data) {
                        $q->where('city', 'like', '%' . $city_data . '%');
                    });
                }
                if (!empty($state_data)) {
                    $mainInventory->whereHas('dealer', function ($q) use ($state_data) {
        ...
        if ($make != null) {
            $mainInventory->where('make', $make);
            $global_make = $make;
        }
        if ($model != null) {
            $mainInventory->where('model', $model);
        }

        $mainInventory->whereNotNull('price')->where('price', '>', 1);
...
//             return $query->paginate(20); // Paginate the results
//         });
...
// }
    ...
    public function tips()
    {
        ...
        if (!empty($firstTips) && !empty($lastTips)) {
            $tips = Tips::where('status', '1')
                ->whereNotIn('id', [$firstTips->id])->limit(12)
                ->get();
        }
    ...
    private function getItemByDistance($zipCode, $zip_radios, $query)
    {
        ...
        // Apply the distance filter using `having`
        $zipCodeQuery->having('distance', '<=', $zip_radios);
        $zipCodeQuery->orderBy('distance', 'asc');
        ...
        \Log::info("Number of results found: " . $inventories->total());
    
        // Check if any results were found
        if ($inventories->isEmpty()) {
            $message = $this->generateNoMatchMessage($zip_radios);
        } else {
            $maxDistance = $inventories->max('distance');
            if ($maxDistance > $zip_radios) {
