    ...
    public function search(Request $request)
    {
        ...
        if ($data) {
            ...
            // $query = MainInventory::with('additionalInventory')->;
                    ...
                    // Initiate main inventory query with eager loading
            $query = MainInventory::select('id', 'deal_id', 'vin', 'year', 'make', 'model', 'price', 'title', 'miles', 'price_rating', 'zip_code', 'latitude', 'longitude', 'payment_price', 'type', 'engine_details', 'inventory_status','transmission')
            ->with([
                'dealer' => function ($query) {
                    $query->select('id', 'dealer_id', 'name', 'state', 'brand_website', 'rating', 'review', 'phone', 'city', 'zip', 'role_id');
                },
        ...
        $infos = $query->where('status', '1')->orderByDesc('id')->paginate(12)->appends(['query' => $data]);
