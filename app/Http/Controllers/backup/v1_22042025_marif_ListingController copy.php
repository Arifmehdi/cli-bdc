...
class ListingController extends Controller
{
    ...
    public function auto(Request $request, $param = null)
    {
        ...
        if ($request->ajax()) {
            ...
            // if($city_name_data == 'Unknown'){
            //     $cacheFilePath = storage_path('app/san_antonio_county.json');
            // }else{
            //     $search_city = $city_name_data.'_county.json';
            //     $cacheFilePath = storage_path('app/'.$search_city);
            // }
// dd($cacheFilePath, $request->all(), $radius, $request->webRadios);
            ...
            if (file_exists($cacheFilePath) && $radiusCalculation < 200) {
                ...
                // $target_make = strtolower($request->input('webMakeFilterMakeInput', '')) ?? strtolower($request->input('secondFilterMakeInputNew', ''));
                // $target_model = strtolower($request->input('webModelFilterInput', '')) ?? strtolower($request->input('secondFilterModelInputNew', ''));
                // $target_body = strtolower($request->input('webBodyFilter', '')) ?? strtolower($request->input('mobileBody', ''));
                // $target_fuel = array_map('strtolower', (array) $request->input('autoWebFuelCheckbox', [])) ?? array_map('strtolower', (array) $request->input('autoMobileFuelCheckbox', []));
                // $target_exterior = array_map('strtolower', (array) $request->input('autoWebExteriorColorCheckbox', [])) ?? array_map('strtolower', (array) $request->input('autoMobileExteriorColorCheckbox', []));
                // $target_interior = array_map('strtolower', (array) $request->input('autoWebInteriorColorCheckbox', [])) ?? array_map('strtolower', (array) $request->input('autoWebInteriorColorCheckbox', []));
                // $target_drivetrain = array_map('strtolower', (array) $request->input('autoWebDriveTrainCheckbox', [])) ?? array_map('strtolower', (array) $request->input('autoMobileDriveTrainCheckbox', []));
                // $target_transmission = array_map('strtolower', (array) $request->input('autoWebTransmissionCheckbox', [])) ?? array_map('strtolower', (array) $request->input('autoMobileTransmissionCheckbox', []));
                // $target_condition= array_map('strtolower', (array) $request->input('autoWebConditionCheckbox', [])) ?? array_map('strtolower', (array) $request->input('autoMobileTypeCheckbox', []));
                // $target_fuel = strtolower($request->input('autoWebFuelCheckbox', []));

// dd(    $maximumPrice = $request->query('maximum_price'), $request->all(), $request->webMakeFilterMakeInput, $request->maximum_price,  $request->input('webMakeFilterMakeInput'), $request->input('maximum_price'));
            ...
            // $target_body = !empty($request->input('webBodyFilter'))
            //     ? strtolower($request->input('webBodyFilter'))
            //     : strtolower($request->input('mobileBody', ''));


            // $target_fuel = !empty($request->input('autoWebFuelCheckbox'))
            //     ? array_map('strtolower', (array) $request->input('autoWebFuelCheckbox'))
            //     : array_map('strtolower', (array) $request->input('autoMobileFuelCheckbox', []));
            ...
            //     // If mobileBody is not empty, use it
            //     $target_fuel = strtolower($request->input('autoMobileFuelCheckbox'));
            // } else {
            //     // If both are empty, use $cookieSetParam or set to null
            ...
            $target_condition = !empty($request->input('autoWebConditionCheckbox'))
                ...
                // dd($request->input('secondFilterMakeInputNew'),$target_make,$target_model,$target_body,$request->all());
                // dd($target_fuel,$target_exterior,$target_interior,$target_drivetrain,$target_transmission,$target_condition,);
...
// dd($request->all(),$target_condition);
                // dd($latitude_infor,$longitude_infor,);
                //     $zip_radios = $firstMatch['zip_code'];


                // $calculate_distance = $this->calculateDistanceData($zip_code_inform, $info , $radius);
                // dd($target_make,$target_model,$target_body,$target_fuel,$target_exterior,$target_interior,$target_drivetrain,$target_transmission,$target_condition,);
                // Apply filters
                $filteredInfo = $info->filter(function ($data) use ($target_make, $target_model, $target_body, $target_fuels, $target_exterior,$target_interior,$target_drivetrain,$target_transmission, $target_condition, $min_price,$max_price,$min_mileage,$max_mileage,$min_year,$max_year,$latitude_infor,$longitude_infor,$zip_radios_inform) {
                    ...
                    // if (!empty($target_body)) {
                    //     if ($target_body === 'electric' || $target_body === 'hybrid') {
                    //         // Filter based on the fuel field for electric or hybrid vehicles
                    //         if (!str_contains(strtolower($data['fuel']), $target_body)) {
                    //             return false;
                    //         }
                    //     } else {
                    //         // Filter based on the body_formated field for other body types
                    //         if (!str_contains(strtolower($data['body_formated']), $target_body)) {
                    //             return false;
                    //         }
                    //     }
                    // }
                    ...
                    // if (!empty($target_fuel) && !str_contains(strtolower($data['fuel']), $target_fuel)) {
                    //     return false;
                    // }
                    ...
                    // // Check if any mapped drivetrain value matches the selected ones
                    // if (!empty($target_drivetrain) && empty(array_intersect(array_map('strtolower', $data_drive_mapped), array_map('strtolower', $target_drivetrain)))) {
                    //     return false;
                    // }
                    ...
                    // rangerMaxPriceSlider =25000
                    // rangerMinPriceSlider=50000

                    // rangerMileageMinPriceSlider =3000
                    // rangerMileageMaxPriceSlider= 5000

                    // rangerYearMinPriceSlider=2008
                    // rangerYearMaxPriceSlider=2020
            ...
            if ($request->has('selected_sort_search')) {
                ...
                // $paginatedItems = array_slice($infor, $offset, $perPage, true);
                // $inventories = new LengthAwarePaginator($paginatedItems, count($infor), $perPage, $page, [
                //     'path' => $request->url(),
                //     'query' => $request->query(),
                // ]);
        ...
        return view('frontend.auto', compact('vehicles', 'vehicles_body', 'searchBody', 'vehicles_fuel_other', 'make_data', 'states', 'cachefuel', 'messageCookieData'))->withCookie($messageCookieData);
