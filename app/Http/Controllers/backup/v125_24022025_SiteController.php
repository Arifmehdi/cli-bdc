...
class SiteController extends Controller
{
    ...
    public function auto(Request $request, $param = null)
    {
        ...
        if ($request->ajax()) {
            ...
            $query->where('price', '>', '1');
            if ($request->weblocationNewInput != null && $request->webRadios != 'Nationwide') {
                $zipCodeData = [
                ...
                $result = $this->getItemByDistance(
                    $zipCodeData['zip_code_data'],
                    $zipCodeData['zip_radios_data'],
                    $zipCodeData['query_data']
                );
            
                $inventories = $result['inventories']; // Get all filtered inventories (77 records)
            // dd($inventories);
                // ✅ Step 1: Limit to 50 items first
                $inventories = $inventories->take(10000); 
            
                // ✅ Step 2: Paginate from these 50 items (20 per page)
                $currentPage = LengthAwarePaginator::resolveCurrentPage();
                $perPage = 20;
                $totalCount = $inventories->count(); // Now always 50 max
            
                $currentItems = $inventories->slice(($currentPage - 1) * $perPage, $perPage)->values();
            
                $inventories = new LengthAwarePaginator(
                    $currentItems,
                    $totalCount, // Correct total count (max 50)
                    $perPage,
                    $currentPage,
                    ['path' => url()->current()]
                );
            
                $message = $result['message']; // Retrieve message
            }
            
            else{
                    // Get all records (limited to 10000 for performance)
    $allInventories = $query->take(10000)->get();

    // Get total count of records after filtering
    $totalCount = $allInventories->count();

    // Apply pagination while preserving the original collection's structure
    $currentPage = LengthAwarePaginator::resolveCurrentPage();
    $perPage = 20;
    ...
    $inventories = new \Illuminate\Pagination\LengthAwarePaginator(
        ...
        $perPage, // Items per page (20 per page)
        $currentPage, // Current page
        ['path' => url()->current()] // Preserve the current URL for pagination links
    );
    ...
    $message = null;
            ...
            // else {
            //     // dd($query->get());
            //     // $allInventories = $query->take(50)->get();  // Limit the query to 50 records
            //     $allInventories = $query->take(10000)->get(); // Get all records (no limit here for count)
    
            //     // Get total count of records after filtering
            //     $totalCount = $allInventories->count(); // Use the actual count here, no hardcoding
                
            //     // Apply pagination
            //     $inventories = new \Illuminate\Pagination\LengthAwarePaginator(
            //         $allInventories->forPage($request->page, 20), // Paginate the 50 records, per page set to 20
            //         $totalCount, // Total number of records
            //         20, // Items per page (20 per page)
            //         $request->page, // Current page
            //         ['path' => url()->current()] // Preserve the current URL for pagination links
            //     );
            //     // $inventories = $query->orderByDesc('id')->limit(50)->get();
            //     $message = null;
            // }
            ...
            // dd($total_count, $current_page_count);
            // $single_inventories_count = ($inventories->perPage() * ($inventories->currentPage() - 1)) + $current_page_count;
            // $single_inventories_count = ($inventories->currentPage() - 1) * 20 + $current_page_count;
            $single_inventories_count = ($request->page - 1) * 20 + $current_page_count;
            // inventory numberf calculation end  here
            $view = view('frontend.auto_ajax', compact('inventories', 'total_count', 'single_inventories_count', 'message','banners'))->render();
    ...
    private function getItemByDistance($zipCode, $zip_radios, $query)
    {
        ...
        if (!$zipCode) {
            return [
                'inventories' => $query->orderByDesc('id')->limit(10000)->get(),
                'message' => $message
            ];
        }
        ...
        try {
            if (!empty($zip_radios) && is_string($zip_radios)) {
                if (isset($zip_location_data['results'][0]['geometry'])) {
                    ...
                    if (!$zipCodeInventories->isEmpty()) {
                        $inventories = $zipCodeQuery->limit(10000)->get();;
                        $maxDistance = $zipCodeInventories->max('distance');
                        ...
                        $inventories = $query->limit(10000)->get();;
                    ...
                    if (isset($zip_location_data['results'][0]['geometry'])) {
                        ...
                        foreach ($radiusOptions as $radius) {
                            ...
                            if ($zipCodeQuery->count() > 0) {
                                $inventories = $zipCodeQuery->orderByDesc('id')->limit(10000)->get();;
                                $foundInventories = true;
                                break;
                            } else {
                                $lastRadiusChecked = $radius;
