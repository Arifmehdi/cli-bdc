...
class SiteController extends Controller
{
    ...
    public function auto(Request $request, $param = null)
    {
        ...
        if ($request->ajax()) {
            ...
            $query->select('id','deal_id','vin','year','make','model','price','title','miles','price_rating','zip_code','latitude','longitude','payment_price','type','engine_details')
            ->with([
                ...
                try {
                    ...
                    if (is_string($zip_radios) && !empty($zip_radios)) {
                        if (isset($zip_location_data['results'][0]['geometry'])) {
                            ...
                            // $zipCodeQuery->selectRaw(
                            //     "main_inventories.id, main_inventories.deal_id, main_inventories.vin, main_inventories.year, main_inventories.make,
                            //      main_inventories.model, main_inventories.price, main_inventories.title, main_inventories.miles,
                            //      main_inventories.price_rating, main_inventories.zip_code,
                            //      (3959 * acos(cos(radians(?)) * cos(radians(latitude)) * cos(radians(longitude) - radians(?)) +
                            //      sin(radians(?)) * sin(radians(latitude)))) AS distance",
                            //     [$latitude, $longitude, $latitude]
                            // );
            ...
            //                 $zipCodeQuery = clone $query;
            //                 $zipCodeQuery->selectRaw(
            //                     "id, deal_id, vin, year, make, model, price, title, miles, price_rating, zip_code,
            //                     (3959 * acos(cos(radians($latitude)) * cos(radians(latitude)) * cos(radians(longitude) - radians($longitude)) + sin(radians($latitude)) * sin(radians(latitude)))) AS distance"
            //                 );
            //                 $zipCodeQuery->having('distance', '<=', $zip_radios);
            //                 $zipCodeQuery->orderBy('distance', 'asc');

            //                 // Get inventories within the radius
            //                 $zipCodeInventories = $zipCodeQuery->get();
            //                 if (!$zipCodeInventories->isEmpty()) {
            //                     $inventories = $zipCodeQuery->paginate(20);
            //                     $maxDistance = $zipCodeInventories->max('distance');
            ...
            //                     if ($maxDistance > $zip_radios) {
            //                         $message = "
            //                             <section class=\"sptb2\" style=\"padding-top: 5px; padding-bottom:3px; margin-bottom:3px\">
            ...
            //                     }
            //                 } else {
            //                     // No results within the radius
            //                     $inventories = $query->paginate(20);
            //                     $message = "
            //                         <section class=\"sptb2\" style=\"padding-top: 5px; padding-bottom:3px; margin-bottom:5px\">
            //                             <div class=\"container bg-white p-5 match\" style=\"border-radius:5px\">
            //                                 <div class=\"text-center\">
            //                                     <h3 class=\"mb-2\">No exact matches within {$zip_radios} miles.</h3>
            //                                     <p class=\"mb-2\">Modify your filters or click \"Save Search\" to be notified when more matches are available.</p>
            //                                     <a href=\"#\" class=\"mb-2 clearfilterAjax\" id=\"clearfilterAjax\" style=\"text-decoration:underline;font-weight:bold;font-size:15px\">Clear all filters.</a>
            //                                 </div>
            //                             </div>
            //                         </section>";
            //                 }
            //             } else {
            //                 // Handle exact zip code search
            //                 $zipCodeQuery = clone $query;
            //                 $zipCodeQuery->where('zip_code', $zipCode);
            //                 $zipCodeInventories = $zipCodeQuery->get();

            //                 if ($zipCodeInventories->isEmpty()) {
            //                     // No exact match, try radius search
            //                     $radiusOptions = [10, 25, 50, 100];
            //                     $foundInventories = false;
            ...
            //                                 <div class=\"container bg-white p-5 match\" style=\"border-radius:5px\">
            //                                     <div class=\"text-center\">
            //                                         <h3 class=\"mb-2\">No matches found within the selected radius.</h3>
            //                                         <p class=\"mb-2\">Modify your filters or click \"Save Search\" to be notified when more matches are available.</p>
            //                                     </div>
            //                                 </div>
            //                             </section>";
            ...
            //             }
            //         } else {
            //             // No location data, fallback to default ordering
            //             $inventories = $query->orderByDesc('id')->paginate(20);
            //         }
            //     } catch (\Exception $e) {
            //         $message = 'An error occurred while processing your request. Please try again.';
            //         Log::error('Error while processing zip code:', ['error' => $e->getMessage()]);
            //         $inventories = $query->orderByDesc('id')->paginate(20);
            //     }
            // } else {
            //     // No zip code provided, default query
            //     $inventories = $query->orderByDesc('id')->paginate(20);
            // }
