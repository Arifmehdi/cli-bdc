...
class SiteController extends Controller
{
    ...
    public function auto(Request $request, $param = null)
    {
        ...
        $price_max = (int)$inventory_obj->where('payment_price', '!=', 'N/A')->max('payment_price');
        ...
        // ********************************************************
        // caching the inventory data start  here
        // Generate cache key
        $search_params = [
            'co_zip' => $request->weblocationNewInput,
            'co_radius' => $request->webRadios,
            'co_make' => $request->webMakeFilterMakeInput,
            'co_model' => $request->webModelFilterInput,
            'co_body' => $request->webBodyFilter,
            'co_min_price' => $request->rangerMinPriceSlider,
            'co_max_price' => $request->rangerMaxPriceSlider,
            'co_min_payment' => $request->input('min_payment'),
            'co_max_payment' => $request->input('max_payment'),
            'co_maximum_miles' => $request->rangerMileageMaxPriceSlider,
            'co_min_year' => $request->rangerYearMinPriceSlider,
            'co_max_year' => $request->rangerYearMaxPriceSlider,
        ];
            $cookie_zip = $request->weblocationNewInput;
            $cookie_radius = $request->webRadios;
            $cookie_make = $request->webMakeFilterMakeInput;
            $cookie_model = $request->webModelFilterInput;
            $cookie_body = $request->webBodyFilter;
            $cookie_min_price = $request->rangerMinPriceSlider;
            $cookie_max_price = $request->rangerMaxPriceSlider;
            $cookie_min_payment = $request->input('min_payment');
            $cookie_max_payment = $request->input('max_payment');
            $co_maximum_miles = $request->rangerMileageMaxPriceSlider;
            $cookie_min_year = $request->rangerYearMinPriceSlider;
            $cookie_max_year = $request->rangerYearMaxPriceSlider;

            $search_key = md5(implode('|', $search_params));

                // Check if cache exists
            $cached_data = DB::table('search_cache')
            ...
            if ($cached_data) {
                    // Use cached data
                    $message = '';
                    // $inventoriesArray = json_decode($cached_data->search_results, true);
                    $inventoriesArray = $this->inventoryService->getItemByFilterByCache($cached_data, $request, $dealer_id);;
                    $filter_options = json_decode($cached_data->filter_options, true);

                    // Convert array to a paginated collection
                    $currentPage = request()->get('page', 1);
                    $perPage = 20; // Adjust this if your original query used a different perPage value
                    $total = count($inventoriesArray);

                    $inventories = new LengthAwarePaginator(
                        collect($inventoriesArray)->forPage($currentPage, $perPage), // Paginate manually
                        $total,
                        $perPage,
                        $currentPage,
                        ['path' => request()->url(), 'query' => request()->query()] // Maintain pagination links
                    );

                    // Now pagination functions will work correctly
                ...
                DB::table('search_cache')->insert([
                    ...
                    'ip' => $ipAddress,
                    'search_results' => json_encode($inventories),
                    'filter_options' => json_encode([
                        ...
                        'model' => $cookie_model,
                        'body' => $cookie_body,
                        'price_max' => $cookie_max_price,
                        'price_min' => $cookie_min_price,
                        'payment_max' => $cookie_max_payment,
                        'payment_min' => $cookie_min_payment,
                        'miles_max' => $co_maximum_miles,
                        'payment_max' => $cookie_max_year,
                        'payment_min' => $cookie_min_year,
                    ]),
                    'updated_at' => now(),
                    'created_at' => now(),
                    'count' => 1,
            ...
            // dd($cached_data, $search_key, $search_params);   
            // caching the inventory data end here 
            // ********************************************************
            // $query = $this->inventoryService->getItemByFilter($request, $dealer_id);
        ...
        // search param send for auto filter start here
        $search_params = [
            'co_zip' => $request->input('zip'),
            'co_radius' => $request->input('radius') ?? 75,
            'co_make' => $request->input('make'),
            'co_model' => $request->input('model'),
            'co_body' => $request->input('body'),
            'co_min_price' => $request->input('min_price'),
            'co_max_price' => $request->input('max_price'),
            'co_min_payment' => $request->input('min_payment'),
            'co_max_payment' => $request->input('max_payment'),
            'co_minimum_miles' => $request->input('minimum_miles') ?? 0,
            'co_maximum_miles' => $request->input('maximum_miles'),
            'co_min_year' => $request->input('min_year'),
            'co_max_year' => $request->input('max_year'),
        ];
        // search param send for auto filter end here
        // dd($search_params);
        ...
        return view('frontend.auto', compact('vehicles', 'vehicles_body', 'searchBody', 'vehicles_fuel_other', 'make_data', 'states', 'cachefuel','search_params'));
