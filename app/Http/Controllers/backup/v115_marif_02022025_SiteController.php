...
class SiteController extends Controller
{
    ...
    public function auto(Request $request, $param = null)
    {
        ...
        if ($request->ajax()) {
            ...
            //     }
            // }
//  jdfgjhd jkgjusdrguih euighuiwehg uwehguh eghuieh guhweugth euithue uwehtu eugth uiet uieyhtuie ytuiyhtueyh tuyeuity weu
            // // **Fallback: If API fails, fetch all cars**
            // if (empty($zip_location_data)) {
            //     $inventories = $query->orderByDesc('id')->paginate(20);
            // } else {
            //     $inventories = $query->orderByDesc('id')->paginate(20);
            //     // // dd($zipCode);
            //     // $location_datar = LocationZip::where('zip_code', $zipCode)->first();
            //     // $zipCodeInventories = null;
            //     // if($location_datar){
            ...
            //     // }
            //     // // **Proceed with filtering based on location**
            ...
            //     //         $message = "<section style=\"padding-top: 5px !important; padding-bottom:3px !important; margin-bottom:3px\" class=\"sptb2\">
            ...
            // Check if the zip code exists in the LocationZip model
            $location = LocationZip::where('zip_code', $zipCode)->first();
            
            if ($location) {
                // If found, use stored latitude and longitude
                $latitude = $location->latitude;
                $longitude = $location->longitude;
            } else {
                ...
                try {
                    ...
                    if (isset($zip_location_data['results'][0]['geometry'])) {
                        ...
                        // Store new ZIP code location in the database for future use
                        LocationZip::create([
                            'zip_code' => $zipCode,
                            'latitude' => $latitude,
                            'longitude' => $longitude
                        ...
                        // If no data found from API, set to null
                        $latitude = null;
                        $longitude = null;
                    }
                } catch (\Exception $e) {
                    // API request failed (limit exceeded, network issue, etc.)
                    $latitude = null;
                    $longitude = null;
                }
            }
            
            // If latitude and longitude exist, calculate distance
            if ($latitude !== null && $longitude !== null) {
                ...
                // If no location found in DB or API, show all
                $inventories = $query->paginate(20);
            }
