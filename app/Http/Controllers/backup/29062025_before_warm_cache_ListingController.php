...
class ListingController extends Controller
{
    ...
    public function __construct(InventoryServiceInterface $inventoryService, DealerService $dealerService)
    {
        ...
        $this->cacheTime = now()->addRealHours((int)env('REDIS_CACHE_TIME', 24)); // Cast to integer
            ...
            config([
            'database.redis.options.maxmemory' => '2gb',
            'database.redis.options.maxmemory-policy' => 'allkeys-lru'
        ]);
    }
    ...
    public function auto(Request $request, $param = null)
    {
        ...
        $selectedSort = $request->input('sort', session('selected_sort_search', 'datecreated|desc'));
        session(['selected_sort_search' => $selectedSort]);
...
protected function directServerQuery($request)
{
    ...
    // Parse URL and query parameters once
    ...
    $mobileLocation = $request->input('mobilelocation');
    $webLocation = $request->input('weblocationNewInput');
    ...
    // Start with the inventory service query
    $query = $this->inventoryService->getItemByFilterOnly($request, $dealer_id);

    // Apply dealer city/state filters if they exist
    if (isset($queryParams['homeDealerCitySearch']) && isset($queryParams['homeDealerStateSearch'])) {
        ...
        $query->whereHas('dealer', function($q) use ($city_data, $state_data) {
            if (!empty($city_data)) {
    ...
    // Optimize the query with specific selects and relationships
    $query->select([
            'id', 'deal_id', 'vin', 'year', 'make', 'model', 'price', 'title',
            'miles', 'price_rating', 'zip_code', 'latitude', 'longitude',
            'payment_price', 'type', 'engine_details'
        ])
        ->with([
            'dealer' => function ($query) {
                $query->select('dealer_id', 'name', 'state', 'brand_website',
                              'rating', 'review', 'phone', 'city', 'zip', 'role_id', 'id');
            },
    ...
    // Handle nationwide vs location-based search
    if ($request->webRadios == 'Nationwide' || $request->mobileRadios == 'Nationwide') {
        $message = null;
        $inventories = $query->paginate(20);
    } else {
        $result = $this->getItemByDistance($zipCode, $zip_radios, $query);
        $inventories = $result['inventories'];
        $message = $result['message'];
    }
    ...
    //         ->with([
    //             'dealer' => function ($query) {
    //                 $query->select('id', 'dealer_id', 'name', 'state', 'phone', 'city', 'zip');
    //             },
    //             'additionalInventory' => function ($query) {
    ...
    // }
    /**
     * Set search parameters in cookies
     */
