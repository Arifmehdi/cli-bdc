...
class SiteController extends Controller
{
    ...
    public function auto(Request $request, $param = null)
    {
        ...
        if ($request->ajax()) {
            ...
            $query->select('id', 'deal_id', 'vin', 'year', 'make', 'model', 'price', 'title', 'miles', 'price_rating', 'zip_code', 'latitude', 'longitude')
                ->with([
            ...
            // // ********************* zip code work fine just go to function start here 
    ...
    private function getItemByDistance($zipCode, $zip_radios, $query)
    {
        ...
        // Check if zip code exists in locationZip table
        ...
        if (!$location) {
            ...
            if (isset($zip_location_data['results'][0]['geometry'])) {
                ...
                $zip_location_city = $zip_location_data['results'][0]['components']['_normalized_city'] ?? '';
                $zip_location_state = $zip_location_data['results'][0]['components']['state'] ?? '';
                $zip_location_state_code = $zip_location_data['results'][0]['components']['state_code'] ?? '';
    
                // Check if required data is available
                if (!empty($zip_location_city) && !empty($zip_location_state) && !empty($zip_location_state_code)) {
    
                    // Insert or get state data
                    $locationState = LocationState::firstOrCreate(
                        ['state_name' => $zip_location_state],
                        [
                            'short_name' => $zip_location_state_code,
                            'sales_tax' => 8.1,
                            'status' => 1
                        ]
                    );
    
                    // Insert or get city data
                    $locationCity = LocationCity::firstOrCreate(
                        ['city_name' => $zip_location_city, 'location_state_id' => $locationState->id],
                        [
                            'latitude' => $latitude,
                            'longitude' => $longitude,
                            'sales_tax' => 8.1,
                    ...
                    // Save to LocationZip table
                    $location = LocationZip::create([
                        'location_city_id' => $locationCity->id,
                        'latitude' => $latitude,
                        'longitude' => $longitude,
                        'zip_code' => $zipCode,
                        'sales_tax' => 8.1,
                ...
                // If API fails, return default results
                return [
                    'inventories' => $query->orderByDesc('id')->paginate(20),
                    'message' => $this->generateNoMatchMessage($zip_radios)
                ];
        ...
        // Ensure $location is set before accessing its properties
        if ($location) {
    ...
    private function getInventoriesByRadius($query, $latitude, $longitude, $zipRadios)
    {
        // Query inventories within a specified radius
        $zipCodeQuery = clone $query;
        $zipCodeQuery->select([
            'main_inventories.id',
            'main_inventories.deal_id',
        ...
        $zipCodeQuery->having('distance', '<=', $zipRadios)->orderBy('distance', 'asc');
        ...
        if ($zipCodeInventories->isEmpty()) {
            return [
                'inventories' => $query->paginate(20),
                'message' => $this->generateNoMatchMessage($zipRadios)
            ];
        }
        ...
        if ($maxDistance > $zipRadios) {
            $message = $this->generateMessage($zipRadios, $maxDistance);
        }
    ...
    private function getInventoriesByExactZip($query, $latitude, $longitude, $zipCode)
    {
        // Query inventories that match the exact zip code
        $zipCodeQuery = clone $query;
        $zipCodeQuery->where('zip_code', $zipCode);
        $zipCodeInventories = $zipCodeQuery->get();
    
        if ($zipCodeInventories->isEmpty()) {
            return $this->findInventoriesWithinRadius($query, $latitude, $longitude);
        }
    ...
    private function findInventoriesWithinRadius($query, $latitude, $longitude)
    {
        // Try different radius options if no exact matches are found
        $radiusOptions = [10, 25, 50, 100];
        foreach ($radiusOptions as $radius) {
            $zipCodeQuery = clone $query;
            $zipCodeQuery->select([
            ...
            $zipCodeQuery->having('distance', '<=', $radius)->orderBy('distance', 'asc');
        ...
        // Return default message if no inventories are found within any radius
        return [
            'inventories' => $query->orderByDesc('id')->paginate(20),
            'message' => $this->generateNoMatchMessage($radius)
        ];
    }
