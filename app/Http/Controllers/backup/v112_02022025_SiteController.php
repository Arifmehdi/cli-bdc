...
class SiteController extends Controller
{
    ...
    {
        // $dealers = User::where('role_id', 'dealer')
        ...
        $query = User::with(['mainInventories'])
            ->where('status', 1) // Only active users
            ->whereHas('roles', function ($query) {
                $query->where('name', 'dealer'); // Only users with role 'dealer'
            })
            ->select('id', 'name', 'phone', 'address', 'city', 'state', 'zip')
            ->withCount('mainInventories') // Count inventories
            // ->having('main_inventories_count', '>', 0)
            ->orderByRaw("
                CASE
                    WHEN name REGEXP '^[0-9]' THEN 1
                    ELSE 0
                END, name ASC
            "); // Sorting: Numbers first, then letters
        ...
        if ($request->ajax()) {
            ...
            $dealers = $query->whereHas('mainInventories') // Only get dealers with maininventories
                        ->paginate(20);
            // dd($dealers);
