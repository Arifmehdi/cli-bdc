...
class SiteController extends Controller
{
    ...
    public function index()
    {
        ...
        $news = News::with('user')->where('status', '1')->select('id','user_id','slug','title','sub_title','img','status')->get();
        ...
        return view('frontend.home', compact('makes', 'makesData', 'years', 'news', 'slider', 'bodies', 'ad1', 'ad2', 'userHistories', 'tendings'));
    }
            ...
            if (!empty($zipCode)) {
                try {
                    ...
                    if (isset($zip_location_data['results'][0]['geometry'])) {
                        ...
                        if (!empty($zip_radios)) {
                            // Query based on user-selected radius
                            ...
                            if ($zipCodeInventories->isEmpty()) {
                                // No exact match, try increasing radii
                        ...
                        // No location data found, fallback to default order
