...
class SiteController extends Controller
{
    ...
    public function auto(Request $request, $param = null)
    {
            ...
            if ($zipCode != null) {
                ...
                try {
                    ...
                    if (is_string($zip_radios) && !empty($zip_radios)) {
                        ...
                        // // home page 100 miles not found issue start here 
                        // $zipCodeQuery = clone $query;
                        // $zipCodeQuery->where('zip_code', $zipCode);
                        // $zipCodeInventories = $zipCodeQuery->get();
                        
                        // // Log the searched ZIP code
                        // Log::info("Searched ZIP Code: {$zipCode}");
                        
                        // // Check if there are matches for the exact ZIP code
                        // if ($zipCodeInventories->isEmpty()) {
                        //     // Call external geolocation API to get latitude and longitude of the ZIP code
                        //     $countryCode = 'us';
                        //     $url = "https://api.opencagedata.com/geocode/v1/json?q={$zipCode},{$countryCode}&key=4b84ff4ad9a74c79ad4a1a945a4e5be1";
                        ...
                        //         // Log the ZIP code coordinates
                        //         Log::info("Searched ZIP Code Coordinates: Latitude: {$latitude}, Longitude: {$longitude}");
                        
                        //         // Perform a single search within 100 miles
                        //         $zipCodeQuery = clone $query;
                        //         $zipCodeQuery->selectRaw(
                        //             "*, (3959 * acos(cos(radians($latitude)) * cos(radians(latitude)) * cos(radians(longitude) - radians($longitude)) + sin(radians($latitude)) * sin(radians(latitude)))) AS distance"
                        //         );
                        //         $zipCodeQuery->having('distance', '<=', 100); // Filter for 100 miles radius
                        //         $zipCodeQuery->orderBy('distance', 'asc');
                        
                        //         // Log the generated SQL query for debugging
                        //         Log::info("Generated Query: " . $zipCodeQuery->toSql());
                        
                        //         $zipCodeInventories = $zipCodeQuery->get();
                        
                        //         // Log the ZIP codes and distances of the results
                        //         foreach ($zipCodeInventories as $result) {
                        //             Log::info("Result ZIP Code: {$result->zip_code}, Distance: {$result->distance}");
                        //             Log::info("Result Coordinates: Latitude: {$result->latitude}, Longitude: {$result->longitude}");
                        //         }
                        
                        //         // Check if any inventories were found within 100 miles
                        //         if ($zipCodeInventories->isEmpty()) {
                        //             // No inventories found within 100 miles, show the message
                        //             $inventories = collect(); // Empty collection to indicate no results
                        //             $message = "
                        //                 <section style=\"padding-top: 5px !important; padding-bottom:3px !important; margin-bottom:5px\" class=\"sptb2\">
                        //                     <div style=\"border-radius:5px\" class=\"container bg-white p-5 match\">
                        //                         <div class=\"text-center\">
                        //                             <h3 class=\"mb-2\">No exact matches within 100 miles.</h3>
                        //                             <p class=\"mb-2\">Modify your filters or click \"Save Search\" to be notified when more matches are available.</p>
                        //                             <a href=\"#\" class=\"mb-2 clearfilterAjax\" style=\"text-decoration:underline;font-weight:bold;font-size:15px\" id=\"clearfilterAjax\">Clear all filters.</a>
                        //                         </div>
                        //                     </div>
                        //                 </section>";
                        //         } else {
                        //             // Inventories found within 100 miles
                        //             $inventories = $zipCodeQuery->orderByDesc('id')->paginate(20);
                        //         }
                        //     } else {
                        //         // Failed to get geolocation data, handle gracefully
                        //         throw new \Exception('Failed to retrieve geolocation data for the provided ZIP code.');
                        //     }
                        // } else {
                        //     // Exact matches found for the ZIP code alone
                        //     foreach ($zipCodeInventories as $result) {
                        //         Log::info("Exact Match ZIP Code: {$result->zip_code}");
                        //     }
                        //     $inventories = $zipCodeQuery->orderByDesc('id')->paginate(20);
                        // }
                        
                        // // home page 100 miles not found issue end here 
                        ...
                        // Check if there are matches for the exact ZIP code
                        if ($zipCodeInventories->isEmpty()) {
                            // Check if latitude and longitude are stored in the database for this ZIP code
                            $location = LocationZip::where('zip_code', $zipCode)->first();
                            ...
                            if ($location) {
                                ...
                                // Log the ZIP code coordinates from the database
                                Log::info("Searched ZIP Code Coordinates from DB: Latitude: {$latitude}, Longitude: {$longitude}");

                                // Perform a search within 100 miles using the Haversine formula
                                $zipCodeQuery = clone $query;
                                $zipCodeQuery->selectRaw(
                                    "*, (3959 * acos(cos(radians($latitude)) * cos(radians(latitude)) * cos(radians(longitude) - radians($longitude)) + sin(radians($latitude)) * sin(radians(latitude)))) AS distance"
                                );
                                ...
                                // Latitude and Longitude not found in DB, call external geolocation API
                                $countryCode = 'us';
                                $url = "https://api.opencagedata.com/geocode/v1/json?q={$zipCode},{$countryCode}&key=4b84ff4ad9a74c79ad4a1a945a4e5be1";
                                $response = file_get_contents($url);
                                $zip_location_data = json_decode($response, true);
                                ...
                                if (isset($zip_location_data['results'][0]['geometry'])) {
                                    ...
                                    // // Log the ZIP code coordinates from the API
                                    // Log::info("Searched ZIP Code Coordinates from API: Latitude: {$latitude}, Longitude: {$longitude}");
        ...
        // $description = "Convenience : Adaptive Cruise Control; Keyless Start; Navigation System; Power Liftgate, Entertainment : Bluetooth®; HomeLink; Premium Sound System; USB Port, Exterior : Alloy Wheels; Sunroof/Moonroof, Safety : Automatic Emergency Braking; Backup Camera; Blind Spot Monitor; Brake Assist; LED Headlights; Lane Departure Warning; Rear Cross Traffic Alert; Stability Control, Seating : Leather Seats; Memory Seat";



        $make_data = $request->input('make');
        $states = LocationState::orderBy('state_name')->pluck('state_name', 'id');
        $cachefuel = Inventory::distinct()->pluck('fuel')->toArray();
    ...
    public function dealerinfo(Request $request, $stockId, $dealer_name, $id = null)
    {
        ...
        $searchBody = $request->query('homeBodySearch');
        $inventores = MainInventory::with('dealer', 'mainPriceHistory', 'additionalInventory')->get();
        $vehicles_obj = VehicleMake::query();

        $vehicles = $vehicles_obj->where('status', 1)->get();
        $inventory_obj = MainInventory::with('dealer', 'mainPriceHistory', 'additionalInventory');
        $vehicles_body = $inventory_obj->distinct()->pluck('body_formated')->toArray();
        $vehicles_fuel_other = $inventory_obj->distinct()->whereNotNull('fuel')->pluck('fuel')->toArray();
        ...
        $make_data = $request->input('make');
        $cachefuel = MainInventory::with('dealer','mainPriceHistory','additionalInventory')->distinct()->pluck('fuel')->toArray();
        return view('frontend.dealer', compact('vehicles', 'vehicles_body', 'inventores', 'searchBody', 'vehicles_fuel_other', 'make_data', 'stockId', 'dealer_name', 'id','cachefuel'));
    }
    ...
    public function showFindDealership(Request $request)
    {
        $query = User::with(['roles', 'mainInventories'])
            ...
            ->select('id', 'dealer_id', 'name', 'phone', 'address')
            ->distinct()
            ->orderByRaw("
            CASE
        ...
        if ($request->ajax()) {
            ...
            $dealersCollection = $dealers->getCollection();
