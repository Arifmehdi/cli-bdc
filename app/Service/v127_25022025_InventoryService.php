...
class InventoryService implements InventoryServiceInterface
{
    ...
    public function getItemByFilterWithOptimized($request, $id = null)
    {
        ...
        $mainInventory = MainInventory::select('id', 'deal_id', 'stock', 'year', 'make', 'model', 'vin', 'active_till', 'featured_till', 'payment_date', 'package','image_count')
            ->with([
    ...
    {
        // dd($request->all());
        // selected_sort_search" => "mileage|desc"
        // selected_sort_search" => "mileage|asc"
        // selected_sort_search" => "searchprice|desc
        // selected_sort_search" => "searchprice|asc
        // selected_sort_search" => "payment|asc
        // selected_sort_search" => "modelyear|desc
            // Dump the request to inspect its structure
    // dd($request->all());

    // Decode the cached JSON data
    // $data = json_decode($cached_data->search_results, true);

    // // If 'selected_sort_search' exists in the request, split it to get the field and order
    // if ($request->has('selected_sort_search')) {
    //     $sortParam = $request->input('selected_sort_search'); // "mileage|desc"
    //     list($field, $direction) = explode('|', $sortParam); // Get the field (e.g., "mileage") and direction (e.g., "asc" or "desc")

    //     // Map the field to the actual data field name
    //     switch ($field) {
    //         case 'mileage':
    //             $field = 'miles'; // miles
    //             break;
    ...
    //             break;
    //         case 'payment':
    //             $field = 'payment_price'; // payment_price
    //             break;
    //         case 'modelyear':
    //             $field = 'year'; // year
    //             break;
    //         default:
    //             // If the field doesn't match, return the data without sorting
    //             return $data;
    //     }
    ...
    //         // Ensure the field exists in both $a and $b
    ...
    // return $data;
        ...
        return json_decode($cached_data->search_results, true);
    }
